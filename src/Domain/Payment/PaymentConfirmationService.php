...
final class PaymentConfirmationService
{
    ...
    public function confirm(string $intentId, array $metadata): PaymentConfirmationResult
    {
        ...
        if ($order === null) {
            return new PaymentConfirmationResult(null, $paymentType, false);
        }
        if ($paymentType === 'full') {
        ...
        $alreadyProcessed = $this->alreadyProcessed($order, $paymentType);
        if (!$alreadyProcessed) {
        ...
        return new PaymentConfirmationResult($order, $paymentType, $alreadyProcessed);
    }
